...
class PIPPatternMiner:
    ...
    def get_permutation_martins(self):
        """
        __len__() for permutation martins
        """
        raise NotImplementedError

    def plot_cluster_examples(self, candle_data: pd.DataFrame, cluster_i: int, grid_size: int = 5):
        """
        After clustering with k-means, color code the clusters to find patterns
        """
        raise NotImplementedError

    def predict(self, pips_y: list):
        """
        Predicting the Clusters based on the pips_y

        - calculate the norm of the pips
        - iterate through each of the cluster_centers
        """
    ...
    def train(self, arr: np.array, n_reps = 1):
        ...
        search_instance = silhouette_ksearch(
            self._unique_pip_patterns, 5, 40, algorithm=silhouette_ksearch_type.KMEANS
        ).process()

        amount = search_instance.get_amount()
        self._kmeans_cluster_pattern(amount)
        ...
        for rep in range(1, n_reps):
            ...
            self._find_unique_patterns()
            search_instance = silhouette_ksearch(
                self._unique_pip_patterns, 5, 40, algorithms=silhoutte_ksearch_type.KMEANS
            ).process()
            amount = search_instance.get_amount()
            self._get_cluster_signals()
            self._assign_clusters()
            perm_martin = self._get_total_perfomance()
            self._perm_martins.append(perm_martin)

    def _find_unique_patterns(self):
        # Find Unique pip pattersn in data
    ...
    def _kmeans_cluster_patterns(self, amount_clusters):
        # Cluster Patterns based on the silhoutte search
        initial_center = kmeans_plusplus_initializer(self._unique_pip_patterns, amount_clusters).initialize()
        kmeans_instance = kmeans(self._unique_pip_patterns, initial_center)
        kmeans_instance.process()

        # Extracting clustering results: clusters and their centers
        self._pip_clusters = kmeans_instance.get_clusters()
        sels._cluster_centers = kmeans_instance.get_centers()


    def _get_cluster_signals(self):
        self._cluster_signals.clear()

        for clust in self._pip_clusters:
            # Loop through each cluster
            signal = np.zeros(len(self._data))
            for mem in clust: # Loop throgh each member in cluster
    ...
    def _assign_clusters(self):
        ...
        # Assign Clusters to long/short intervals
        clusters_martins = []
...
if __name__ == "__main__":
    ...
    pip_miner = PIPPatternMiner(n_pips=5, lookback=24, hold_period=5)
